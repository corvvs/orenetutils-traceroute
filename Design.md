...
1. プログラム全体の情報をもつもの
    - `ft_traceroute`は1つしかDstを取らないが, もし複数取れるしたらどうかを考える
2. dstセッションの情報をもつもの
3. ホップセッションの情報をもつもの
4. ホップラウンドの情報をもつもの

それぞれの構造体が持っていて欲しい情報を考えてみる。
...
### ホップセッションにおける送受信制御のメカニズム

並列処理オプションの実装を前提とするなら, ホップセッションでは複数のホップラウンドが同時に走るので、
送受信処理が入り乱れることになる。

ホップラウンドは「プローブの送信」によって始まり, 「対応する(と思われる)データグラムの受信」か「タイムアウト」で終わる。

イベント監視系関数の利用が禁止されているため, イベント監視には他の手が必要。\
→ ソケットのタイムアウトオプションを使う。\
ソケットにタイムアウトを設定して受信を処理を行うと

- データの受信 or エラー
- タイムアウト到達
- SIGINT
    - macOSだとSIGINTでは返らないっぽい

で受信関数が返るので, そのタイミングでイベントの発生をチェックする。

#### セットすべきタイムアウト

以下のうちもっとも小さいもの

- 今走っているホップラウンドのタイムアウト時刻のうち最小値 - 現在時刻
- イベント監視粒度(1秒とか0.1秒とか)
